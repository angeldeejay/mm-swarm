...
let mmPath = '/home/node/MagicMirror';
let mmpmPath = '/home/node/mmpm';
...
if (fs.existsSync(mmpmPath + '/mmpm-external-packages.json') &&
  fs.existsSync(mmpmPath + '/mmpm-external-packages.json') &&
  fs.statSync(mmpmPath + '/mmpm-external-packages.json').isFile()) {
  try {
    Object.values(JSON.parse(fs.readFileSync(mmpmPath + '/mmpm-external-packages.json')))
...
if (fs.existsSync(mmpmPath + '/MagicMirror-3rd-party-packages-db.json') &&
  ...
  try {
    Object.values(JSON.parse(fs.readFileSync(mmpmPath + '/MagicMirror-3rd-party-packages-db.json')))
...
fs.readdirSync(mmPath + '/modules', { withFileTypes: true })
  ...
  .forEach((file, index) => {
    ...
    if (fs.existsSync(definitionsPath) &&
      ...
      packageData = { ...defaultPackageData, ...JSON.parse(fs.readFileSync(definitionsPath)) };
    }
    packageData = (({ title, author, repository, description }) => ({ title, author, repository, description }))(packageData);

    if (fs.existsSync(modulePath + '/.git') && fs.statSync(modulePath + '/.git').isDirectory()) {
      try {
        ...
        if (typeof repoData['remote "origin"'] !== 'undefined') {
          ...
          packageData.repository = `${repoData['remote "origin"'].url}`
            .trim()
            .replace(/^([\w]+@)?([^:]+):([^\/]+)\/(.+)(\.git)?$/ig, 'https://$2/$3/$4')
            .replace(/\.git$/ig, '');
        }
      } catch (_) { }
    }
    for (const k of Object.keys(defaultPackageData)) {
      if (packageData[k] !== defaultPackageData[k] && packageData[k].trim() === '') {
...
if (shouldSaveExternalPackages) {
  console.log('► Saving ' + externalPackages.length + ' external packages found');
  fs.writeFileSync(mmpmPath + '/mmpm-external-packages.json', JSON.stringify({ "External Packages": externalPackages }, null, 4));
}
console.log('► Done');
